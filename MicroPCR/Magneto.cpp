...
CString CMagneto::protocolCompile(vector<CString> &protocol){
	...
	static const CString tempCmdList[12] = { L"goto", L"filter", L"mixing", L"waiting", L"pumping up", L"pumping down", L"ready", L"home", L"magnet on", L"magnet off", L"heating", L"pcr", };
	...
	for (int i = 0; i < protocol.size(); ++i)
	{
		...
		if( cmd.Compare(L"pumping") == 0 )
		{
			...
			if( subCmd.Compare(L"up") == 0 || subCmd.Compare(L"down") == 0 )
				cmd += L" " + subCmd;
			else
			{
...
void CMagneto::initPredefinedAction()
{
	...
	preDefinedAction[ProtocolCmd::WAIT].push_back(ActionBinary(ActionCmd::WAIT, 1, INT_MAX));

	// PUMPING UP Command argument 값을 넣어준다.
	preDefinedAction[ProtocolCmd::PUMPING_UP].push_back(ActionBinary(ActionCmd::MOVE_ABS, 3, MotorType::PUMPING, INT_MAX, M_PUMPING_SPEED));
	...
	preDefinedAction[ProtocolCmd::READY].push_back(ActionBinary(ActionCmd::MOVE_ABS, 3, MotorType::CHAMBER,  23505, M_CHAMBER_SPEED));
	...
	preDefinedAction[ProtocolCmd::HOME].push_back(ActionBinary(ActionCmd::SECOND_WAIT, 1, MotorType::CHAMBER));
	...
	preDefinedAction[ProtocolCmd::MAGNET_ON].push_back(ActionBinary(ActionCmd::MAGNET_ON, 1, INT_MAX));
	
	// MAGNET OFF Command argument 값을 넣어준다.
	preDefinedAction[ProtocolCmd::MAGNET_OFF].push_back(ActionBinary(ActionCmd::MAGNET_OFF, 1, INT_MAX));
...
void CMagneto::initDriverParameter(){
	...
	*/
	...
	if (FAS_SetParameter(comPortNo, MotorType::PUMPING, 21,1) != FMM_OK)		// PUMPING 축 home 방향 변경(CCW->CW)
		driverErrCnt++;
	if (FAS_SetParameter(comPortNo, MotorType::PUMPING, 17,50000) != FMM_OK)	// PUMPING 축 home 속도 변경(50000)
		driverErrCnt++;
	if (FAS_SetParameter(comPortNo, MotorType::PUMPING, 18,10000) != FMM_OK)	// PUMPING 축 serach speed 변경(10000)
		driverErrCnt++;
}
	...
	for (int i = 0; i < protcolBinary.size(); ++i)
	{
		...
		for (int j = 0; j < ab.size(); ++j)
		{
			...
			if (pb.cmd == ProtocolCmd::GO){
				static int _hallNum = 0;
				int chamber = pb.arg - 1;
				int position = 0;

				if(_hallNum <= chamber)
					position = (chamber*M_CHAMBER_INTERVAL)+M_CHAMBER_DIFF+M_CHAMBER_OFFSET - M_CHAMBER_DISK_OFFSET - 32000;
				else if( _hallNum > chamber)
					position = (chamber*M_CHAMBER_INTERVAL)+M_CHAMBER_DIFF+M_CHAMBER_OFFSET - M_CHAMBER_DISK_OFFSET - M_CHAMBER_BACKLASH - 32000;
			...
			*/
				for (int k = 0; k < tempAb.args.size(); ++k)
				{
					if (tempAb.args[k] == Magneto::DefaultPos)
					{
						if(pb.cmd = ProtocolCmd::PUMPING_UP)
						{
							int volume = pb.arg;
						...
						else if (pb.cmd = ProtocolCmd::PUMPING_DOWN)
						{	
							int volume = pb.arg;
							if( volume == -1)
							{
							...
							{
								long currentPos = 0;
								if(FAS_GetCommandPos(comPortNo, MotorType::PUMPING , &currentPos) != FMM_OK)
									return;
								tempAb.args[k] = currentPos + (M_PUMPING_STEP_PER_REV * volume) / (PI * M_PUMPING_DISK_RADIUS * M_PUMPING_DISK_RADIUS) + M_PUMPING_POS_UP_OFFSET;
			...
			if( !( (tempAb.cmd == ActionCmd::PCR) || (tempAb.cmd == ActionCmd::MAGNET_ON) || (tempAb.cmd == ActionCmd::HEATING) || (tempAb.cmd == ActionCmd::MAGNET_OFF) ) )
			{
				...
				{
					double a = pulse2mili(((MotorType::Enum)tempAb.args[0]), tempAb.args[1]);
					double a1 = pulse2mili(((MotorType::Enum)tempAb.args[0]), tempAb.args[1]);
					child.Format(L"%s, %s, %0.3lf, %d", 
			...
			{
				if(tempAb.cmd == ActionCmd::HEATING)
				{
					tempAb.args[0] = pb.arg;
				}
				child.Format(L"-");
			}
...
bool CMagneto::isActionFinished()
{
	...
	if( !( (ab.cmd == ActionCmd::PCR) || (ab.cmd == ActionCmd::MAGNET_ON) || (ab.cmd == ActionCmd::HEATING) || (ab.cmd == ActionCmd::MAGNET_OFF) ) )
	{
...
void CMagneto::runNextAction()
{
	...
	switch (cmd)
	{
		...
		case ActionCmd::PCR:
			...
			break;
		case ActionCmd::HEATING:
			::SendMessage(hwnd, WM_HEATING_CHANGED, ab.args[0], 0);
			break;
		case ActionCmd::MAGNET_ON:
			::SendMessage(hwnd, WM_MAGNET_CHANGED, 1, 0);
			break;
		case ActionCmd::MAGNET_OFF:
			::SendMessage(hwnd, WM_MAGNET_CHANGED, 0, 0);
			break;
		case ActionCmd::SECOND_WAIT:
