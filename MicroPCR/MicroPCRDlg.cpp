...
CMicroPCRDlg::CMicroPCRDlg(CWnd* pParent /*=NULL*/)
	...
	, openConstants(false)	// false 로 한 후, 함수를 호출하여 열도록 한다. 
	, openGraphView(false)
	, isConnected(false)
	, m_cMaxActions(1000)
	, m_cTimeOut(1000)
	...
	if (!magneto->isConnected())
	{
		if (idx != -1)
		{
			...
			if (res == DriverStatus::CONNECTED){
				...
				AfxMessageBox(L"Magneto와 연결되었습니다.\n하단의 아두이노와 연결하세요.");
			...
			AfxMessageBox(L"Com port 를 선택하세요.");
	}
...
void CMicroPCRDlg::OnBnClickedButtonConnectArduino()
{
	...
	//
	//AfxMessageBox(L"Magneto 와 연결되었습니다.\n하단 Start 버튼을 누르면 Test 가 진행됩니다.");
}
